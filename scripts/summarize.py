...
def render(root,run):
    ...
    if project=='A':
        ...
        lines += [f'{len(check["records"])} GPU cases passed. Each case runs in an isolated process with disable_split_kv=True. The automatic split-KV path retains a reproduced merge-kernel registration diagnostic. Both independent output and base-2 LSE comparisons pass; guards and caller buffer identity pass.',
